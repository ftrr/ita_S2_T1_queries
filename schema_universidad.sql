...
/*9*/
SELECT DISTINCT persona.nombre, persona.apellido1
FROM persona
JOIN alumno_se_matricula_asignatura
...
ON pf.id_departamento = departamento.id

RIGHT JOIN persona
ON persona.id = pf.id_profesor
...
/*2*/
SELECT persona.apellido1, persona.apellido2,persona.nombre, pf.id_departamento
FROM persona

LEFT JOIN profesor pf
ON pf.id_profesor = persona.id

WHERE persona.tipo = 'profesor'
AND pf.id_departamento IS NULL
...
SELECT DISTINCT departamento.nombre, profesor.id_profesor
FROM departamento
...
/*5*/
SELECT grado.nombre as grado, COUNT(asignatura.id_grado) as nº_asignaturas
FROM grado
...
/*8*/
SELECT  curso_escolar.anyo_inicio ,  COUNT( DISTINCT am.id_alumno)
FROM curso_escolar 

LEFT JOIN alumno_se_matricula_asignatura am
    ON am.id_curso_escolar = curso_escolar.id

GROUP BY curso_escolar.anyo_inicio;

/*9*/
SELECT profesor.id_profesor, persona.nombre, persona.apellido1, persona.apellido2, COUNT(asignatura.id_profesor) as nº_asignaturas
FROM profesor 
...
WHERE persona.tipo='profesor' AND
    asignatura.id_profesor IS NULL

GROUP BY persona.nombre, persona.apellido1;
